...
enum domainType { None, Alphabetic, Numeric, AlphaNumeric }

class EmailValidator {
  static int _index = 0;
  static const String _atomCharacters = "!#\$%&'*+-/=?^_`{|}~";
  static domainType _domainType = domainType.None;
  ...
  static bool _isDomain(String c, bool allowInternational) {
    if (c.codeUnitAt(0) < 128) {
      if (_isLetter(c) || c == '-') {
        _domainType = domainType.Alphabetic;
        return true;
      }

      if (_isDigit(c)) {
        _domainType = domainType.Numeric;
        return true;
      }
  ...
  static bool _skipDomain(
    ...
    if (_domainType == domainType.Numeric) {
      return false;
    }
